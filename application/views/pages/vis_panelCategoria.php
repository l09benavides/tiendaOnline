...
<!DOCTYPE html>
<!--//************************************************************************************
//Este módulo corresponde a la vista de Categorias que se muestra a los usuarios con rol de
// administrador y provee la funcionalidad CRUD para los registro de Categorias.
//Autor: Dany Alvarado
//Fecha de creación: 02/8/2018
//Lista modificaciones
//20/08/2018 Gabriel Aleman
...
<ol class="breadcrumb">
    ...
    <div class="container">
        <h1 style="font-size:20pt">PANEL CATEGORIAS</h1>

        <h3>Categorias</h3>
        <br />
        <button class="btn btn-success" onclick="add_categoria()"><i class="glyphicon glyphicon-plus"></i> Agregar Categoria</button>
...
// Funcion JavaScrip que permite llamar y utilizar los metodos del controller Cntrl_panelCategoria.php

$(document).ready(function() {

    // Define el formato de la tabla categorias
    table = $('#table').DataTable({ 
       ...
       //Cambio de idioma Table
        ...
        "ajax": {
            "url": "<?php echo site_url('Ctrl_panelCategoria/ajax_list')?>",
            "type": "POST"
        },
        ...
        "columnDefs": [
            { 
                "targets": [ -1 ], //Ultima columna
                "orderable": false, //conjunto no ordenable de columna
            },
            { 
                "targets": [ -2 ], //2 Ultima columna (Foto)
                "orderable": false, //Conjunto no ordenable de columna
            },
        ],
...
//Método ajax que permite agregar nuevas categorias por medio de la funcion save al utilizar la el valor almacenado
//en la variable save_method que define la condicion de lo que realiza la funcion save() que posteriormente llamara el //controlador //Ctrl_panelCategoria/ajax_add 
function add_categoria() 
{
    ...
    $('.modal-title').text('Agregar Categoria'); //Establecer el título en el título modal Bootstrap
...
//Método ajax que permite editar las categorias al pasar el parametro id al controlador Ctrl_panelCategoria/ajax_edit.
function edit_categoria(id)
{
    ...
    $.ajax({
        url : "<?php echo site_url('Ctrl_panelCategoria/ajax_edit')?>/" + id,
        type: "GET",
        dataType: "JSON",
        success: function(data)
        {
            ...
            $('.modal-title').text('Editar Categoria'); //Establecer título al modal de Bootstrap
...
//en un registro de Categorias.
function reload_table()
{
    table.ajax.reload(null,false); //Recargar datatable ajax 
}

//Método ajax que permite guardar registros nuevos o actualizar las categorias al llamar el controlador //Ctrl_panelCategoria/ajax_add - Ctrl_panelCategoria/ajax_update
function save()
{

    $('#Detalle').val(CKEDITOR.instances.Detalle.getData());
    $('#btnSave').text('Guardando...'); //Cambio el texto o etiqueta del button que ve el usuario cuando se guarda
    $('#btnSave').attr('disabled',true);//Habilito el button 
    var url;

    if(save_method == 'add') {
        url = "<?php echo site_url('Ctrl_panelCategoria/ajax_add')?>";
    } else {
        url = "<?php echo site_url('Ctrl_panelCategoria/ajax_update')?>";
    }
...
//Método ajax que permite borrar las categorias al pasar el parametro id al controlador Ctrl_panelCategoria/ajax_delete.
function delete_categoria(id)
{
    if(confirm('Esta seguro que desea Eliminar está Categoria?'))
    {
        //Ajax borrar datos a la base de datos
        $.ajax({
            url : "<?php echo site_url('Ctrl_panelCategoria/ajax_delete')?>/"+id,
            type: "POST",
            dataType: "JSON",
            success: function(data)
            {
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h3 class="modal-title">Categoria Formulario</h3>
